        ...
        switch (tipo) {
            case 'cadastro':
                var [rows, fields] = await pool.query(
                    "insert into `harley`.`tbl_login` (`nome`, `login`, `senha`) values (?, ?, ?);",
                    [nome, login, senha]
                );
            ...
            case 'login':
                var [rows, fields] = await pool.query(
                    "select * from `harley`.`tbl_login` where `nome` = ? and `login` = ? and `senha` = ?;",
                    [nome, login, senha]
                );
                if (rows.length == 1) {
            ...
            case 'leitura':
                ...
                var strSql = "select * from `harley`.`tbl_login` where" + 
